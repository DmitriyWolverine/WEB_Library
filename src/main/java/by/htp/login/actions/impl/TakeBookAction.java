...
public class TakeBookAction implements BaseAction{
	private int curBookId;
	
	private BookService bookService;
	private UserService userService;
	
	public TakeBookAction() {
	...
	private boolean mainActions(HttpServletRequest request, User user) {
		...
		if(bookAvailable() && !userService.hasAnyTakenBooks(user)){
			userService.initCurrentBook(user, bookService.getBookById(curBookId));
			userService.addBookToList(user, bookService.getBookById(curBookId));
			bookService.changeStatusBook(curBookId, false);
